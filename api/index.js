...
// node_modules/.deno/@hono+node-server@1.19.17/node_modules/@hono/node-server/dist/index.mjs
...
var PanService = class {
  ...
  async findPanByAadhaar(aadhaar) {
    ...
    if (response.Errorcode === 100 && response.Data?.PanNumber) {
      ...
      logger2.info(`[PanService] Found PAN: ${response.Data}`);
      return {
        pan,
        maskedPan
      };
    ...
    logger2.warn(`[PanService] Find PAN failed: ${failureReason}`);
...
var serverless_default = getRequestListener(app.fetch);
export {
  config,
