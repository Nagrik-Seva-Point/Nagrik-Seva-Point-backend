...
import type { ServiceInput } from "../integrations/integration.interface.ts";
...
export class RequestService {
  ...
  async processRequest(requestId: string, organizationId: string) {
    ...
    if (request.service.code !== "PAN_FIND") {
      throw AppError.badRequest(
        `Unsupported service: ${request.service.code}`,
        "UNSUPPORTED_SERVICE",
      );
    }

    // 1. Validate inputs
    await panAdapter.validateInput(request.inputData as ServiceInput);
    ...
    try {
      ...
      const result = await panAdapter.execute(
        request.inputData as ServiceInput,
      );
      ...
      if (result.success) {
        ...
        await requestRepository.updateResult(requestId, organizationId, {
          error: result.error,
        });
        const updatedRequest = await requestRepository.updateStatus(
          requestId,
      ...
      await requestRepository.updateResult(requestId, organizationId, {
        error: "Internal integration gateway timeout/failure",
      });
      const updatedRequest = await requestRepository.updateStatus(
        requestId,
        organizationId,
